...
class AsistenciaController extends BaseController {
    ...
    public function justificar($id) {
        $this->requireAuth();
        $stmt = $this->db->prepare("SELECT a.*, s.cédula FROM asistencias a JOIN socios s ON a.id_socio = s.id_socio WHERE a.id_asistencia = ?");
        $stmt->execute([$id]);
        $asis = $stmt->fetch();
        if (!$asis) $this->json(['error' => 'No encontrada'], 404);
        if ($asis['cédula'] !== ($_SESSION['usuario_cedula'] ?? '')) $this->json(['error' => 'No autorizado'], 403);
        ...
        if ($archivo) {
            $this->db->prepare("UPDATE asistencias SET justificación = ?, justificación_pdf = ? WHERE id_asistencia = ?")->execute([$texto, $archivo, $id]);
        } else {
            $this->db->prepare("UPDATE asistencias SET justificación = ? WHERE id_asistencia = ?")->execute([$texto, $id]);
        }
    ...
    public function aprobarJustificacion($id) {
        ...
        $this->db->prepare("UPDATE asistencias SET justificación_aprobada = ? WHERE id_asistencia = ?")->execute([$aprobada, $id]);
    ...
    public function listar() {
        ...
        if ($filtroSesion) { $where[] = 'a.id_sesión = ?'; $params[] = $filtroSesion; }
        $whereClause = $where ? 'WHERE ' . implode(' AND ', $where) : '';

        $stmt = $this->db->prepare("SELECT a.*, CONCAT_WS(' ', s.apellido1, s.apellido2, s.nombre1, s.nombre2) AS socio,
                                     s.cédula, ses.número_sesión, ses.fecha AS fecha_sesión
                                     FROM asistencias a
        ...
        $sesiones = $this->db->query("SELECT id_sesión, número_sesión, fecha FROM sesiones_mensuales ORDER BY fecha DESC LIMIT 20")->fetchAll();
